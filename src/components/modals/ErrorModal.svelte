<script lang="ts">
  import { Bug, Copy, Check, ExternalLink, Info, Settings, Monitor, HardDrive } from 'lucide-svelte';
...
<Modal
  ...
  titleIcon={Bug}
  titleClass="bg-red-500/10 text-red-400"
>
  {#if error}
  ...
  {#snippet footer()}
    <Button variant="ghost" full onclick={handleCopy}>
      {#if copied}
    ...
    <Button variant="gradient" full onclick={handleReportIssue}>
